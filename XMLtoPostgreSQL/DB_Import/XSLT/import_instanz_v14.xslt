...
<xsl:stylesheet version="2.0" xmlns:gmd="http://www.isotc211.org/2005/gmd" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:math="/java.lang.Math" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2004/10/xpath-functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:OpenInfRA="OpenInfRA" xmlns:uuid="java:java.util.UUID">
	...
	<xsl:template match="/*">
		<xsl:result-document method="text" href="SQL_Script/Import_instanz.sql">
			...
			<xsl:apply-templates select="//OpenInfRA:attribute_type_group"/>
			<xsl:apply-templates select="//OpenInfRA:attribute_type_group/OpenInfRA:subgroup_of"/>
			...
			<xsl:apply-templates select="//OpenInfRA:attribute_type_group_to_topic_characteristic"/>
			...
			<xsl:apply-templates select="//OpenInfRA:attribute_type_to_attribute_type_group"/>
	...
	<xsl:template match="//OpenInfRA:attribute_type/OpenInfRA:rel_attribute_type">
		...
		<xsl:value-of select="concat('INSERT INTO &quot;attribute_type_x_attribute_type&quot; VALUES (',$apos,$rel_id,$apos ,',' ,$Id, ', ' ,$at2, ', ' , $SKOS, ');')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>
	
	<xsl:template match="//OpenInfRA:attribute_type_group">
		<xsl:variable name="Id" select="concat($apos,OpenInfRA:id,$apos)"/>
		<xsl:variable name="name">
			<xsl:for-each select="OpenInfRA:name/gmd:PT_FreeText/gmd:textGroup/@xlink:href">
				<xsl:if test="position() = 1">
		...
		<xsl:variable name="description">
			<xsl:choose>
				<xsl:when test="OpenInfRA:description">
					<xsl:for-each select="OpenInfRA:description/@value_list_value">
						<xsl:if test="position() = 1">
							<xsl:variable name="sub" select="substring-after(.,'uuid_')"/>
							<xsl:value-of select="concat($apos,$sub,$apos)"/>  
						</xsl:if>
		...
		<xsl:value-of select="concat('INSERT INTO &quot;attribute_type_group&quot; VALUES (' ,$Id, ', ' ,$name, ', ' , $description, ', ' ,$null,');')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>	
	
	<xsl:template match="//OpenInfRA:attribute_type_group/OpenInfRA:subgroup_of">
		<xsl:variable name="id" select="concat($apos,parent::*/OpenInfRA:id,$apos)"/>
		<xsl:variable name="sub">
			<xsl:choose>
				<xsl:when test="OpenInfRA:reference">
		...
		<xsl:variable name="subgroup" select="concat($apos,$sub,$apos)"/>
		<xsl:value-of select="concat('UPDATE &quot;attribute_type_group&quot; SET &quot;subgroup_of&quot; = ' ,$subgroup, ' WHERE  &quot;id&quot; =' ,$id,';')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>
	...
	<xsl:template match="//OpenInfRA:project">
		<xsl:if test="@id">
			<xsl:variable name="Id" select="concat($apos,substring-after($spath,'project_'),$apos)"/>
			<xsl:variable name="name">
				<xsl:for-each select="OpenInfRA:name/gmd:PT_FreeText/gmd:textGroup/@xlink:href">
					<xsl:if test="position() = 1">
						<xsl:variable name="sub" select="substring-after(.,'uuid_')"/>
	...
	<xsl:template match="//OpenInfRA:topic_characteristic">
		...
		<xsl:variable name="project" select="concat($apos,substring-after($spath,'project_'),$apos)"/>
	...
	<xsl:template match="//OpenInfRA:attribute_type_group_to_topic_characteristic">
		<xsl:variable name="sub">
			<xsl:choose>
		...
		<xsl:variable name="sub2">
			<xsl:choose>
				<xsl:when test="OpenInfRA:rel_attribute_type_group/OpenInfRA:reference">
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group/OpenInfRA:reference/@idref,'uuid_')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group/OpenInfRA:attribute_type/@id,'uuid_')"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="attribute_type_group" select="concat($apos,$sub2,$apos)"/>
		...
		<xsl:variable name="order">
			<xsl:choose>
				<xsl:when test="OpenInfRA:order">
					<xsl:variable name="sub4" select="OpenInfRA:order"/>
		...
		<xsl:variable name="sub5" select="substring-after(@id,'uuid_')"/>
		<xsl:variable name="Id" select="concat($apos,$sub5,$apos)"/>
		<xsl:value-of select="concat('INSERT INTO &quot;attribute_type_group_to_topic_characteristic&quot; VALUES (' ,$Id, ',' ,$attribute_type_group, ', ' ,$topic_characteristic, ', ' , $multi, ', ' ,$order,');')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>
	...
	<xsl:template match="//OpenInfRA:relationship_type_to_topic_characteristic">
		...
		<xsl:variable name="Id" select="concat($apos,$sub5,$apos)"/>
		<xsl:variable name="multi" select="concat($apos,$sub3,$apos)"/>
		<xsl:value-of select="concat('INSERT INTO &quot;relationship_type_to_topic_characteristic&quot; VALUES (',$Id,', ' ,$topic_characteristic, ', ' ,$relationship_type, ', ' , $multi, ');')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>
	...
	<xsl:template match="//OpenInfRA:attribute_type_to_attribute_type_group">
		<xsl:variable name="sub">
			<xsl:choose>
				<xsl:when test="OpenInfRA:rel_attribute_type_group/OpenInfRA:reference">
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group/OpenInfRA:reference/@idref,'uuid_')"/>
				...
				<xsl:otherwise>
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group/OpenInfRA:attribute_type_group/@id,'uuid_')"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="attribute_type_group" select="concat($apos,$sub,$apos)"/>
		<xsl:variable name="sub2">
			<xsl:choose>
		...
		<xsl:variable name="sub3">
			<xsl:choose>
				<xsl:when test="OpenInfRA:rel_attribute_type_group_to_topic_characteristic/OpenInfRA:reference">
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group_to_topic_characteristic/OpenInfRA:reference/@idref,'uuid_')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_group_to_topic_characteristic/OpenInfRA:attribute_type_group_to_topic_characteristic/@id,'uuid_')"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="attribute_type_group_to_topic_characteristic" select="concat($apos,$sub3,$apos)"/>
		...
		<xsl:variable name="multi" select="concat($apos,$sub4,$apos)"/>
		...
		<xsl:variable name="order">
			<xsl:choose>
				<xsl:when test="OpenInfRA:order">
					<xsl:variable name="sub5" select="OpenInfRA:order"/>
					<xsl:value-of select="concat($apos,$sub5,$apos)"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="'NULL'"/>	
				</xsl:otherwise>
		...
		<xsl:variable name="sub6" select="substring-after(@id,'uuid_')"/>
		<xsl:variable name="Id" select="concat($apos,$sub6,$apos)"/>
		<xsl:value-of select="concat('INSERT INTO &quot;attribute_type_to_attribute_type_group&quot; VALUES (', $Id, ',' ,$attribute_type, ', ' ,$attribute_type_group, ', ' , $attribute_type_group_to_topic_characteristic, ', ' ,$multi, ',' ,$default, ',' ,$order,');')"/>
		<xsl:text>&#10;</xsl:text>
	</xsl:template>
	...
	<xsl:template match="//OpenInfRA:attribute_value">
		...
		<xsl:variable name="sub2">
			<xsl:choose>
				<xsl:when test="OpenInfRA:rel_attribute_type_to_attribute_type_group/OpenInfRA:reference">
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_to_attribute_type_group/OpenInfRA:reference/@idref,'uuid_')"/>
				</xsl:when>
				<xsl:otherwise>
					<xsl:value-of select="substring-after(OpenInfRA:rel_attribute_type_to_attribute_type_group/OpenInfRA:attribute_type_to_attribute_type_group/@id,'uuid_')"/>
				</xsl:otherwise>
			</xsl:choose>
		</xsl:variable>
		<xsl:variable name="attribute_type_to_attribute_type_group" select="concat($apos,$sub2,$apos)"/>
		<xsl:variable name="sub3">
			<xsl:choose>
		...
		<xsl:if test="OpenInfRA:domain">
			...
			<xsl:value-of select="concat('INSERT INTO &quot;attribute_value_domain&quot; VALUES (' ,$Id, ', ' ,$attribute_type_to_attribute_type_group, ', ' , $topic_instance,  ', ' ,$domain, ');')"/>
			<xsl:text>&#10;</xsl:text>
		</xsl:if>
		<xsl:if test="OpenInfRA:value">
			...
			<xsl:value-of select="concat('INSERT INTO &quot;attribute_value_value&quot; VALUES (' ,$Id, ', ' ,$attribute_type_to_attribute_type_group, ', ' , $topic_instance,  ', ' ,$value, ');')"/>
			<xsl:text>&#10;</xsl:text>
		</xsl:if>
		<xsl:if test="OpenInfRA:geometry">
			...
			<xsl:value-of select="concat('INSERT INTO &quot;attribute_value_geom&quot; VALUES (' ,$Id, ', ' ,$attribute_type_to_attribute_type_group, ', ' , $topic_instance,  ', ' ,'ST_GeomFromGML(',$apos,normalize-space($geom),$apos, '));')"/>
			<xsl:text>&#10;</xsl:text>
		</xsl:if>
		<xsl:if test="OpenInfRA:geometryZ">
			...
			<xsl:value-of select="concat('INSERT INTO &quot;attribute_value_geomz&quot; VALUES (' ,$Id, ', ' ,$attribute_type_to_attribute_type_group, ', ' , $topic_instance,  ', ' ,'ST_GeomFromGML(',$apos,normalize-space($geomZ),$apos, '));')"/>
			<xsl:text>&#10;</xsl:text>
		</xsl:if>
	</xsl:template>
