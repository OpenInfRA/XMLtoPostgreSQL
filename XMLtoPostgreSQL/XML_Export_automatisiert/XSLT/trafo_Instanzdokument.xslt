...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:math="/java.lang.Math" xmlns:xlink="http://www.w3.org/1999/xlink" xmlns:fn="http://www.w3.org/2004/10/xpath-functions" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns="OpenInfRA" xmlns:gml="http://www.opengis.net/gml">
	...
	<xsl:template match="/*">
		<xsl:result-document method="xml" href="Ergebnis_XML/OpenInfRA_XML.xml" encoding="UTF-8" indent="yes">   <!--Generieren der Zieldatei OpenInfR_XML.xml -->
			<xsl:element name="dataset" >  <!-- Wurzelelement -->
				...
				<xsl:attribute name="xsi:schemaLocation" select="'OpenInfRA ../../XSD/OpenInfRA_xsd_v12.xsd'" namespace="http://www.w3.org/2001/XMLSchema-instance"/> <!-- Hinzufügen des relativen Speicherpfades der XSchema Datei -->
				...
				<xsl:apply-templates select="attribute_type_group/attribute_type_group[not(.=following::attribute_type_group/attribute_type_group)]"/>	
				<!-- Aufruf des Templates zum Erzeugen der attribute_type_to_topic_characteristic=================== -->		    
				<xsl:apply-templates select="attribute_type_to_topic_characteristic"/>
	...
	<xsl:template match="attribute_type_group/attribute_type_group[not(.=following::attribute_type_group/attribute_type_group)]"> <!-- nachfolgende Anweisungen für jedes attribute_type_groupn Element der Klasse attribute_type_group Element, jedoch nur einmal für jede attribute_type_group -->
		<xsl:element name="attribute_type_group"> <!-- Erzeugen des Elements attribute_type_group -->
			<xsl:variable name="id"> <!-- Schreiben der id in eine Variable -->
				<xsl:value-of select="."/>
			</xsl:variable>
			...
			<!-- Hinzufügen der Beziehung zu attribute_type========================================= -->
			<xsl:for-each select="key('attribute_type_attribute_type_group_by_attribute_type_group',$id)"> <!-- nachfolgende Anweisungen werden für jedes attribute_type_group Element durchführt, welches den entsprechenden Schlüssel (key = attribute_type_group) aufweist -->
				<xsl:element name="rel_attribute_type"> <!-- Erzeugen des Elements rel_Attribut -->
					<xsl:element name="reference">
						<xsl:attribute name="idref"> <!-- Hinzufügen des Attributes idref -->
	...
	<xsl:template match="attribute_type_to_topic_characteristic">
		<xsl:element name="attribute_type_to_topic_characteristic">
